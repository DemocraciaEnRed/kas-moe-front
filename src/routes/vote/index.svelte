    ...
    <div class="stepper">
      <div class="flex gap-4 items-center">
        ...
        <h3 class="text-xl text-moe-blue">
          Selecciona la plancha que quieres votar
        </h3>
      </div>
      <div>
        ...
        <div class="card">
          <div class="card-header">
            <div class="flex items-center gap-4">
              <span>
                <img src={item.logo} alt="Logotipo de la lista">
              </span>
              <div>
                <h2>Plancha</h2>
                <h2>
            ...
            {#each item.group as member}
            <div class="flex gap-4">
              <img src={member.avatar} alt="Foto de perfil">
              <div>
                <h4>{member.name}</h4>
                <p class="font-noto">{member.role}</p>
              </div>
            </div>
            {/each}
            </div>
...
<style>
  ...
  .content {
    @apply h-full p-16 flex flex-col justify-center items-center bg-moe-cyan;
  }

  .stepper {
    @apply w-8/12 h-20 p-6 mb-8 flex justify-between items-center bg-moe-cyan-dark rounded-md;
  }

  .step {
    @apply w-8/12 flex flex-col gap-4 h-auto;
  }

  .card {
    @apply w-full h-80 flex bg-white rounded-lg;
    box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  }

  .card-header {
    @apply w-9/12 p-6 flex flex-col;
  }
  ...
  h4 {
    @apply text-lg font-sand font-bold;
  }

  .card-action {
    @apply w-3/12 self-center text-center;
  }
</style>